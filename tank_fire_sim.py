...
APP_MODES: Dict[str, tuple] = {
    # Группа А — без обучения
    "imitation": ("📽  ИМИТАЦИЯ",                   "#566573", "#eaecee"),
    "trainer":   ("🎓  ТРЕНАЖЁР РТП",               "#8e44ad", "#f3e5f5"),
    "sppр":      ("🧭  СППР",                       "#1a6da8", "#e3f0fb"),
    # Группа Б — обучение без reward
    "cloning":   ("📋  КЛОНИРОВАНИЕ ПОВЕДЕНИЯ",     "#d68910", "#fef9e7"),
    "irl":       ("🔄  ОБРАТНОЕ ОП",                "#a04000", "#fbeee6"),
    # Группа В — обучение с reward
    "rl_flat":   ("🤖  ТАБЛИЧНОЕ ОП",               "#1e8449", "#e8f8f0"),
    "rl_hier":   ("🏛  ИЕРАРХИЧЕСКОЕ ОП (моно)",    "#1a5276", "#d6eaf8"),
    "rl_multi":  ("👥  МУЛЬТИАГЕНТНОЕ ОП",           "#0e6655", "#d0ece7"),
    # Группа Г — на данных / обратной связи
    "offline":   ("📦  ПАКЕТНОЕ ОП",                "#7d3c98", "#f4ecf7"),
    "pref":      ("⚖  ОП ПО ПРЕДПОЧТЕНИЯМ",        "#117a65", "#d1f2eb"),
}
...
class TankFireSim:
    ...
    def _apply_scripted(self, t: int, desc: str):
        ...
        if "пнс" in lo and ("реке" in lo or "мост" in lo or "водоисточник" in lo or "открытый" in lo):
    ...
    def _update_phase(self):
        ...
        if self.extinguished:
            ...
            # РВС средний: пороги по хронологии ПТП (мин от начала)
...
class ModeSelectDialog(tk.Tk):
    ...
    _GROUPS = [
        ("А. Без обучения", "#95a5a6", [
            ("imitation", "📽", "Имитация",
             "#566573",
             "Воспроизведение сценария\nпо хронологии без агента",
             ["Автоматический выбор действий",
              "Наблюдение за динамикой пожара",
              "Базовая линия для сравнения"]),
            ("trainer", "🎓", "Тренажёр РТП",
             "#8e44ad",
            ...
            ("sppр", "🧭", "СППР",
             ...
             ["Рекомендация + уверенность %",
              "Журнал отклонений",
              "Отчёт о согласии человек-ИИ"]),
        ]),
        ("Б. Обучение без функции вознаграждения", "#d4ac0d", [
        ...
        ("В. Обучение с функцией вознаграждения", "#27ae60", [
            ...
            ("rl_hier", "🏛", "Иерархическое\n(моноагентное)",
             "#1a5276",
             "Один агент на уровень\nстратегия → тактика → операции",
             ["L3→L2→L1: один агент на уровень",
              "Учебная программа: от простого",
              "Сравнение с табличным агентом"]),
            ("rl_multi", "👥", "Мультиагентное\nобучение",
             "#0e6655",
             "РТП + 3 НБУ + НТ = 5 агентов\nлокальные наблюдения, общие ресурсы",
             ["5 агентов: РТП, НБУ×3, НТ",
              "Секторы: юг, восток, запад",
    ...
    # ── Подробные описания с примерами для каждого режима ────────────────────
    _MODE_HELP: Dict[str, str] = {
        "imitation": (
            "ИМИТАЦИЯ — воспроизведение реального сценария пожара\n\n"
            "Для чего: наблюдение за развитием пожара без вмешательства. "
            "Система автоматически выбирает действия по экспертным правилам "
            "(приоритеты действий определяются фазой пожара). Функция "
            "вознаграждения не используется, агент не обучается.\n\n"
            "Пример: выберите сценарий «РВС крупный (сценарий А, 81 ч)», "
            "нажмите ▶ Пуск. Система проиграет все 48 событий хронологии: "
            "прибытие подразделений, развёртывание стволов, пенные атаки, "
            "ликвидацию. На карте вы увидите динамику: как растёт число "
            "техники, как меняется площадь пожара. Результат — базовая линия "
            "для сравнения с обученными агентами.\n\n"
            "Кому полезен: преподавателям для демонстрации, "
            "исследователям для получения эталонных метрик."
        ),
        "trainer": (
            "ТРЕНАЖЁР РТП — оценка компетентности руководителя тушения пожара\n\n"
            "Для чего: курсант самостоятельно принимает решения на каждом шаге "
            "симуляции, а система сравнивает его выбор с оптимальным действием "
            "обученного агента и выставляет баллы.\n\n"
            "Система оценки:\n"
            "  · Точное совпадение с лучшим действием: +20 баллов\n"
            "  · Действие в тройке лучших: +10 баллов\n"
            "  · Допустимое, но не лучшее: +2 балла\n"
            "  · Недопустимое в текущей фазе: −5 баллов\n\n"
            "Пример: в фазе S2 (разведка) вам предлагается 15 действий. "
            "Вы выбираете «О1 — Подача Антенор-1500 на охлаждение». "
            "Агент считает это лучшим действием → +20. Если бы вы выбрали "
            "«С4 — Пенная атака», а техника ещё не прибыла → −5.\n\n"
            "По итогам формируется отчёт с разбором каждого решения."
        ),
        "sppр": (
            "СППР — система поддержки принятия решений\n\n"
            "Для чего: помощь действующему руководителю тушения пожара. "
            "Обученный агент на каждом шаге выдаёт рекомендацию с уровнем "
            "уверенности. Оператор принимает совет или выбирает другое действие. "
            "Все отклонения фиксируются в журнале.\n\n"
            "Пример: агент рекомендует «Т3 — Вызов дополнительных сил» с "
            "уверенностью 87%. Вы видите, что силы уже на подходе, и нажимаете "
            "«Переопределить» → выбираете «О1 — Охлаждение горящего РВС». "
            "Система запишет: отклонение в фазе S3, ваш выбор vs рекомендация.\n\n"
            "Научная ценность: метрика «уровень согласия человек-ИИ» показывает, "
            "в каких фазах функция вознаграждения может быть неточной."
        ),
        "cloning": (
            "КЛОНИРОВАНИЕ ПОВЕДЕНИЯ — обучение без функции вознаграждения\n\n"
            "Для чего: обучить агента копировать решения опытного РТП. "
            "Датасет собирается автоматически из режимов «Тренажёр» и «СППР»: "
            "каждое решение пользователя записывается как пара "
            "(состояние → действие).\n\n"
            "Пример: после 5 сеансов в тренажёре накопилось 200 записей. "
            "Переключаетесь в режим «Клонирование» → нажимаете «Обучить». "
            "Система подсчитывает частоты: в состоянии X эксперт выбирал "
            "действие A в 80% случаев. Результат — таблица вероятностей.\n\n"
            "Научная ценность: ответ на вопрос «Достаточно ли записей, "
            "чтобы обучить агента без проектирования функции вознаграждения?» "
            "Проблема: агент хорошо работает в знакомых фазах, но теряется "
            "в редких ситуациях (S4, S5)."
        ),
        "irl": (
            "ОБРАТНОЕ ОБУЧЕНИЕ — восстановление функции вознаграждения\n\n"
            "Для чего: понять, что на самом деле оптимизирует опытный "
            "руководитель тушения. Алгоритм анализирует экспертные траектории "
            "и восстанавливает веса: насколько важна площадь пожара, "
            "безопасность людей, стоимость действий, качество реагирования.\n\n"
            "Пример: из записей ПТП (сценарий А, сценарий Б) система извлекает "
            "траектории решений. После обучения получаем: «Тяжесть пожара: 0.40, "
            "Дефицит ресурсов: 0.30, Стоимость действия: 0.05, Качество: 0.25». "
            "Сравниваем с ручными весами (0.35, 0.25, 0.10, 0.30) — видим, "
            "что эксперт придаёт больше значения ресурсам и меньше — стоимости.\n\n"
            "Научная ценность: явная формулировка критерия эффективности РТП, "
            "извлечённая из данных, а не заданная вручную."
        ),
        "rl_flat": (
            "ТАБЛИЧНОЕ ОБУЧЕНИЕ С ПОДКРЕПЛЕНИЕМ — базовый алгоритм\n\n"
            "Для чего: обучить агента методом проб и ошибок. Агент "
            "взаимодействует с симулятором, получает вознаграждение за каждое "
            "действие и постепенно запоминает оптимальную стратегию в Q-таблице "
            "(315 состояний × 15 действий).\n\n"
            "Пример: запустите обучение на 1000 эпизодов. На графике видно: "
            "первые 200 эпизодов агент действует случайно (ε=0.90), "
            "к 500-му эпизоду уже уверенно выбирает охлаждение в фазе S2 "
            "и пенную атаку в S4, к 800-му — ε снизился до 0.10.\n\n"
            "На тепловой карте Q-значений видно: в критических состояниях "
            "агент ценит «О3 — Пенная атака» выше всего, "
            "а в начальных фазах — «О4 — Разведка» и «Т3 — Вызов сил».\n\n"
            "Это базовый алгоритм для сравнения с иерархическим агентом."
        ),
        "rl_multi": (
            "МУЛЬТИАГЕНТНОЕ ОБУЧЕНИЕ С ПОДКРЕПЛЕНИЕМ — 5 агентов\n\n"
            "Для чего: исследование координации между несколькими агентами, "
            "действующими одновременно на разных участках пожара. В отличие "
            "от иерархического моноагентного режима (один агент на уровень), "
            "здесь 5 агентов с ЛОКАЛЬНЫМИ наблюдениями и ОБЩИМИ ресурсами.\n\n"
            "Агенты:\n"
            "  · РТП (1) — стратегия: выбор цели для всех НБУ\n"
            "  · НБУ-1 (юг), НБУ-2 (восток), НБУ-3 (запад) — по 8 действий\n"
            "  · НТ (1) — распределение воды и пены между секторами\n\n"
            "Пример: РТП ставит цель «Оборона». НБУ-1 подаёт ствол на юге, "
            "НБУ-2 готовит пенную атаку с востока, НБУ-3 охлаждает с запада. "
            "НТ направляет 50% воды на НБУ-2 (приоритетный сектор). Если НБУ-1 "
            "вместо обороны начинает разведку — это отклонение (α_НБУ-1 растёт).\n\n"
            "Научная ценность: как координация влияет на эффективность? "
            "При каком α_НБУ система работает лучше? Как добавление 4-го "
            "и 5-го БУ влияет на скорость ликвидации?"
        ),
        "rl_hier": (
            "ИЕРАРХИЧЕСКОЕ МОНОАГЕНТНОЕ ОП — 3 уровня, один агент на уровень\n\n"
            "Для чего: обучить агента, который повторяет реальную структуру "
            "управления пожаром — стратегия (НГ МЧС), тактика (РТП), "
            "оперативные действия (НБТП).\n\n"
            "Три уровня:\n"
            "  · Уровень 3 (каждые 30 мин): выбор режима управления — "
            "собственные силы / региональные / федеральные\n"
            "  · Уровень 2 (каждые 10 мин): тактическая цель — "
            "штурм / оборона / наращивание / разведка / эвакуация\n"
            "  · Уровень 1 (каждые 5 мин): конкретное действие из 15\n\n"
            "Пример: учебная программа обучает сначала на простом сценарии "
            "(РВС средний, 5 ч), затем на смешанном, затем на сложном "
            "(РВС крупный, 81 ч). Через 1000 эпизодов иерархический агент "
            "на 15–20% эффективнее плоского по скорости ликвидации.\n\n"
            "Сравнение с табличным агентом выводится в статистической таблице "
            "с доверительными интервалами и критерием значимости."
        ),
        "offline": (
            "ПАКЕТНОЕ ОБУЧЕНИЕ — обучение на исторических данных\n\n"
            "Для чего: обучить агента на фиксированном датасете переходов "
            "(состояние, действие, награда, следующее состояние) без "
            "взаимодействия с симулятором. Полезно, когда есть записи "
            "реальных пожаров, но запускать симулятор многократно невозможно.\n\n"
            "Пример: из 10 прогонов в режимах «Имитация» и «Табличное ОП» "
            "накоплено 5000 переходов. Переключаетесь в «Пакетное обучение», "
            "загружаете датасет, запускаете 50 итераций. Система обновляет "
            "Q-таблицу только на имеющихся данных.\n\n"
            "Научная ценность: ответ на вопрос «Можно ли обучить агента "
            "только на записях реальных пожаров МЧС?» Сравнение с онлайн-агентом "
            "показывает потерю качества от отсутствия исследования."
        ),
        "pref": (
            "ОБУЧЕНИЕ ПО ПРЕДПОЧТЕНИЯМ ЭКСПЕРТА — калибровка вознаграждения\n\n"
            "Для чего: улучшить функцию вознаграждения через обратную связь "
            "от эксперта. Система показывает пары вариантов развития событий, "
            "эксперт указывает, какой лучше. Из предпочтений восстанавливается "
            "модель вознаграждения.\n\n"
            "Пример: в режиме СППР оператор 30 раз отклонил рекомендацию "
            "агента. Каждое отклонение = неявное предпочтение (выбор оператора "
            "лучше, чем совет агента). Система обучает модель: в каких "
            "состояниях оператор выше ценит определённые действия.\n\n"
            "После обучения: модель предпочтений совпадает с Q-таблицей "
            "агента в 75% состояний. Расхождения — области, где функция "
            "вознаграждения неточна. Агент дообучается на откалиброванной модели.\n\n"
            "Научная ценность: как сделать так, чтобы ИИ действительно "
            "оптимизировал то, что хочет человек."
        ),
    }
    ...
    def _build(self):
        ...
        utilities = [
            ("📂", "Импорт базы пожаров", "#1abc9c",
             "Загрузка описаний реальных пожаров из папки",
             self._run_import),
            ("🏭", "Генератор сценариев", "#d4ac0d",
             "Создание синтетических сценариев для всех модулей",
             self._run_generator),
            ("📄", "Отчётные материалы", "#e74c3c",
             "Генерация отчётов, экспорт, визуализации",
    ...
    def _run_reports_window(self):
        ...
        reports = [
            ("📑", "Научная статья",
             "Полная статья с рисунками, таблицами и формулами",
             "#c0392b", self._run_article),
            ("📄", "Отчёт по симуляции",
             "Параметры, хронология, графики, нормативный анализ",
             "#2980b9", lambda: self._tool_message("Отчёт",
             "Запустите симуляцию, затем сгенерируйте отчёт из меню.")),
            ("📊", "Экспорт данных",
             "Прецеденты, статистика, корреляции для внешних инструментов",
             "#27ae60", self._run_export),
            ("📐", "Таблицы для диссертации",
             "Форматированные таблицы для вставки в текст работы",
             "#e67e22", self._run_export),
            ("📋", "Схема диссертации",
             "Структурно-логическая схема (10 уровней)",
             "#8e44ad", self._run_dissertation_scheme),
            ("🗺", "Схема карты пожара",
             "Все 16 элементов визуализации обстановки",
             "#1abc9c", self._run_map_scheme),
            ("📖", "Руководство пользователя",
             "Полное описание функций платформы",
    ...
    def _make_card(self, parent, mode_key, icon, title, accent, desc, features,
        ...
        help_text = self._MODE_HELP.get(mode_key, "")
        default_text = "Наведите курсор на карточку режима для подробного описания с примером."
        ...
        def _on_enter(e, c=card, ht=help_text):
            _set_bg(c, BG_H)
            self._desc_var.set(ht)

        def _on_leave(e, c=card):
            _set_bg(c, BG_N)
            self._desc_var.set(default_text)
...
class TankFireApp(tk.Tk):
    ...
    # Маппинг новых режимов → базовые категории GUI (для переиспользования логики)
    _MODE_CATEGORY = {
        "imitation": "imitation",
        "trainer":   "trainer",
        "sppр":      "sppр",
        "cloning":   "research",    # использует research UI + вкладка клонирования
        "irl":       "research",    # использует research UI + вкладка IRL
        "rl_flat":   "research",    # research UI, фокус на Flat RL
        "rl_hier":   "research",    # research UI, фокус на иерарх. моноагентное
        "rl_multi":  "research",    # research UI, фокус на мультиагентное
        "offline":   "research",    # research UI + offline вкладка
        "pref":      "research",    # research UI + вкладка предпочтений
    }

    def __init__(self, mode: str = "rl_flat"):
        super().__init__()
        self._mode = mode
        self._mode_cat = self._MODE_CATEGORY.get(mode, "research")
    ...
    def _build_ui(self):
        ...
        _label, _accent, _bg_clr = APP_MODES.get(self._mode, APP_MODES["rl_flat"])
        ...
        _mode_hints = {
            "imitation": "Наблюдение за динамикой пожара — агент выбирает экспертные действия",
            "trainer":   "Выберите действие РТП кнопками ниже карты",
            "sppр":      "Примите или отклоните рекомендацию агента",
            "cloning":   "Обучение агента на записях эксперта (без функции вознаграждения)",
            "irl":       "Восстановление функции вознаграждения из экспертных траекторий",
            "rl_flat":   "Обучение плоского Q-агента методом проб и ошибок",
            "rl_hier":   "Иерархическое моноагентное обучение (L3→L2→L1, один агент на уровень)",
            "rl_multi":  "Мультиагентное обучение (РТП + 3 НБУ + НТ, локальные наблюдения, общие ресурсы)",
            "offline":   "Обучение на фиксированном датасете без взаимодействия с симулятором",
            "pref":      "Калибровка reward через сравнение пар траекторий экспертом",
        }
        tk.Label(mode_bar, text=f"│  {_mode_hints.get(self._mode, '')}",
                 font=("Arial", 9),
    ...
    def _build_settings_tab(self, parent):
        ...
        _manual_text = (
            ...
            "    РВС средний (Ранг 2): РВС-9, топливо = нефть, S₀=1250 м², плавающая крыша 70%.\n"
            "    РВС крупный (Ранг 4): РВС-20000, топливо = мазут, S₀=2000 м², плавающая крыша 70%.\n"
            "• Параметры пожара:\n"
    ...
    def _build_controls(self):
        ...
        _ctrl_hints = {
            "imitation": "  Нажмите ▶ Пуск для воспроизведения сценария (наблюдение без вмешательства)",
        ...
        _ctrl_hints.setdefault(self._mode, "  ▶ Пуск — запуск симуляции  |  Обучение и инструменты — через верхнее меню")
        tk.Label(ctrl,
    ...
    def _build_workflow_guide(self):
        ...
        _steps_by_mode = {
            "imitation": [
                ("1", "Настройки",  "⚙ Меню → Настройки →\nВыбор сценария",         self._open_settings_window),
                ("2", "Пуск",       "▶ Воспроизведение\nсценария по хронологии",      "timeline"),
                ("3", "Динамика",   "📊 Графики метрик\nв реальном времени",         "metrics"),
            ],
            "trainer": [
        ...
        steps = _steps_by_mode.get(self._mode_cat, _steps_by_mode["research"])
    ...
    def _draw_map(self):
        ...
        wy_top = H - 52
        if is_tuapse:
            # ── РВС крупный: открытый водоисточник (река) ─────────────────────
            ...
            self._map_label(c, W//2, wy_top+42,
                            "городская улица — водопроводная сеть (давление ≥ 0.15 МПа)",
                            "#444", ("Arial", 7, "italic"), bg="#b0b0a8")

        # ══════════════════════════════════════════════════════════════════════
        # 3. ПРИЧАЛ / ПАНРК (РВС крупный) — перетаскиваемый
        # ══════════════════════════════════════════════════════════════════════
        if is_tuapse:
            _pw, _ph = 64, 38
        ...
        if is_tuapse:
            ...
            # РВС средний — Насосная — перетаскиваемая
            _pw, _ph = 78, 38
    ...
    def _animate(self):
        ...
        # ── ИМИТАЦИЯ / СППР / RL / все остальные: стандартный цикл ─────────
        ended = False
        for _ in range(self._speed):
            if self.sim.t >= self.sim._cfg["total_min"] or self.sim.extinguished:
                ended = True
                break
            # Режим «Имитация»: действие по экспертным правилам
            ...
            elif self._mode == "sppр" and self._sppр_override_pending is not None:
